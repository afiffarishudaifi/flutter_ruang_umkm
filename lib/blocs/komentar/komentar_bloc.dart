  ...
  KomentarBloc() : super(KomentarLoadedState(komentars: [])) {
    on<KomentarLoadEvent>((event, emit) async {
      Map<String, dynamic> komentar =
          await KomentarService().loadKomentar(event);
      if (komentar.containsKey("errorCode")) {
      } else {
        List<Komentar> komentars = komentar['response']
            .map<Komentar>((json) => Komentar.fromJson(json))
            .toList();
        emit(KomentarLoadedState(
            komentars: komentars, isError: false, errorMessage: ''));
      }
    });
